...
export function FooterSection() {
  return (
    <footer className="w-full border-t border-purple-900/30 bg-[#040408] py-10 px-6 relative z-10">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        {/* Brand Logo & Tagline */}
        <div className="flex flex-col items-center md:items-start space-y-1.5">
          <Logo variant="navbar" href="/" />
          <p className="text-xs text-slate-400 font-mono">
            Built by <span className="text-purple-300 font-semibold">LogicLoom</span>
          </p>
        </div>
        ...
        <nav className="flex flex-wrap items-center justify-center gap-6 text-xs font-mono text-slate-400">
          ...
          <Link href="#built-by" className="hover:text-white transition-colors">
            Built By
          </Link>
        ...
        <div className="flex flex-col items-center md:items-end space-y-1.5">
          ...
          <span className="text-[11px] font-mono text-slate-500">
            © {new Date().getFullYear()} INTERVUE AI • Built by LogicLoom
          </span>
        </div>
      </div>
    </footer>
